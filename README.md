...
## 🆕 v2.6 — 컷 체인(연속성 고정): 앞 컷 마지막 프레임을 다음 컷의 시작점으로

30초·60초 씬을 15초 컷으로 쪼개면 얼굴은 같아도 컷마다 카메라 높이·소품 상태·서 있는 자리가 다시 잡혀 이음새에서 "튀는" 문제를 정식 단계로 해결했습니다.

- **`scripts/last_frame.sh`** — 렌더된 컷의 진짜 마지막 프레임을 `continuity/`에 추출 (페이드·블러 끝이면 `LAST_OFFSET`으로 살짝 앞 프레임).
- **워크플로 §6a 신설** — 같은 장소에서 시간 점프 없이 이어지는 컷은 필수 체인: 앞 컷 렌더·QC 통과 → 마지막 프레임 추출 → 업로드 → 다음 컷에 투입 → 프롬프트 첫 줄 "Frame opens matching the final frame of CUT N: <상태 요약>".
- **이음새 종류로 투입 방식 선택** — 같은 샷이 이어지면 Seedance 2.5 `start_image`(첫 프레임 고정), 앵글이 바뀌는 하드컷이면 continuity Element(기하·위치·소품 상태만, 앵글 제외), 원테이크가 관건이면 `video_extension`.
- **project.json `continuity_ref`** — 컷마다 앵커 출처·미디어 id·투입 방식을 기록. 앞 컷을 재렌더하면 체인 아래 컷은 자동으로 stale → 재추출·재렌더.
- **QC 항목 추가** — 체인 컷은 첫 프레임을 앞 컷 마지막 프레임과 나란히 비교(`qc.continuity`).
- **v2.6.1 (실전 피드백 반영)** — 이음새는 앞 컷 마지막 샷을 재현하지 않고 **다른 앵글·동작 중간**으로 연다(`start_image` 같은 샷 잇기는 원테이크 전용). 컷 끝 "Hold." 금지. **대화 컷 페이싱 하한**: 15초에 대사 2~3줄, 3~4초 샷 4~5개, 3초 초과 무대사 정지 샷 금지, ★TEMPO LOCK★(샷은 액션·대사 끝 0.5초 안에 컷, 의도적 멈춤 컷당 1회). 원안 대사가 희박하면 프롬프트 전 대사 보강 패스.

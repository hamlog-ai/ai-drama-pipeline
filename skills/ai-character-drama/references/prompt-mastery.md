...
- **밀도 하한 (2026-09-18 도어록 교훈).** 상한만 지키고 하한을 안 정했더니
  120초에 대사 11줄, 컷마다 "쳐다본다 → 홀드"가 붙어 "호흡이 너무 길다, 말 없는
  정지 장면이 너무 많다"는 시사 판정을 받았다. 대화 씬의 15초 컷은 **대사 최소
  2줄** (상한 3줄은 그대로), **3초를 넘는 무대사 정지 샷 금지**, 리액션 전용 샷
  ("A를 보고 B를 본다")은 그 컷의 펀치일 때만 1개. 침묵은 샷으로 만들지 말고
  초 단위로 써라 ("1초 정적 후"). 원안 대사가 희박하면 프롬프트 전에 **대사
  보강 패스**를 돌린다: 플롯을 바꾸지 않는 짧은 반응 대사("...네?", "잠깐만",
  "누구셔?")를 컷당 1~2줄 추가하거나, 컷을 10초로 압축한다 — 둘 중 하나를 골라
  바이블에 기록하고 사용자에게 보여준다.
...
### 비트 예산: 액션 컷 15초 = 3비트 / 대화 컷 15초 = 4~5비트

- 비트 하나에는 **물리적 액션 1개 + 대사 1줄**. 그 이상 욱여넣으면 뭉개진다.
- **템포 (2026-09-18 도어록 교훈: "컷이 너무 느려, 쓸데없는 호흡이 너무 길다").**
  5초 비트 3개는 액션·VFX 컷용이다. **대화 씬은 3~4초 비트 4~5개**로 쪼개고, 샷은
  액션이나 대사가 끝나는 순간에서 0.5초 안에 자른다 — 여운·정착·"카메라가 머문다"
  금지. 대사와 대사 사이 정적은 1초 이내, "한 박자 쉬고 / a full second passes /
  a small swallow before" 같은 의도적 멈춤은 **컷당 1회**만. 다음 화자는 앞 대사
  꼬리를 물고 들어온다 ("she answers before he finishes"). 리액션은 대사 위에
  겹쳐 보여주지, 대사 뒤에 따로 3초 샷으로 만들지 않는다. 밀도가 아쉬우면 액션을
  줄이지 말고 **대사를 늘려라**(대사 밀도 하한 참조) — 리액션 샷을 추가하는 건
  더 이상 기본 처방이 아니다.
- 프롬프트에 **★TEMPO LOCK★**을 세운다: "Brisk tempo: every shot ends within 0.5s
  of its action or line; no lingering holds; pauses between lines under 1s
  except ONE deliberate beat at <시점>; the next speaker comes in on the tail
  of the previous line." FORMAT MODE에도 "brisk, no dead air"를 선언한다.
...
- 마지막 비트는 가급적 **리액션/표정**으로 끝내되, **정지 홀드("Hold.")로 끝내지
  마라** — 특히 같은 장소로 이어지는 체인 컷은 동작이나 대사 꼬리 *위에서* 잘라
  다음 컷이 그 움직임을 받게 한다. 홀드로 끝나고 홀드로 열리면 이음새마다
  정지→재시동이 생겨 툭툭 끊긴다 (도어록 2026-09-18).
...
[ ] 비트 예산: 비트당 액션 1개 이하? 액션 컷 3비트 / 대화 컷 4~5비트(3~4초)?
[ ] 템포: ★TEMPO LOCK★ 있나? 의도적 멈춤 컷당 1회 이하? "hold / lingers / settles / a full second passes"가 2회 이상 남아 있지 않나?
[ ] 대사 예산: 컷당 ≤3줄, 줄당 ≤15음절, 구어체, 화자 ≤2명?
[ ] 대사 밀도: 대화 씬이면 컷당 ≥2줄? 3초 초과 무대사 정지 샷 없음? 리액션 전용 샷 ≤1?
[ ] 체인 컷: 앞 컷 마지막 샷을 재현하지 않고 다른 앵글·동작 중간으로 여는가? 끝이 "Hold."가 아닌가?
